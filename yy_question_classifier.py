...
CORPUS_DIR = "classifier_output_data_1"
...
flags.DEFINE_string("export_dir", "test/output/"+CORPUS_DIR+"/bert_model", "The dir where the exported model will be written.")
...
class YYDataProcessor(DataProcessor):
    ...
    def _createexamples(self, data_path, max_count = 100):
        ...
        # examples1 = reader[360:460]
        # examples2 = reader[870:970]
        # examples3 = reader[1120:1170]
        # reader = examples1 + examples2 + examples3
        examples = []
    ...
    def get_train_examples(self, data_dir):
        ...
        print("----------读train数据-----------")
        examples = self._createexamples(file_path, 0)
        print("----------读train数据 end-----------")
        return examples
    ...
    def get_labels(self):
        return ['how','query','why','what','whether']
...
def main(_):
  ...
  if FLAGS.do_predict:
    print("开始测试")
    estimator = tf.contrib.predictor.from_saved_model('test/output/classifier_output_data_1/bert_model/1587553445')
    ...
    for (ex_index, example) in enumerate(predict_examples):
        ...
        probility = prediction['probabilities']
        max_probbility_index = np.argmax(probility)
        result = label_list[max_probbility_index]
        test_csv_writer.writerow([example.text_a, result])
